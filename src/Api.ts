...
export function fetchEqpList() {
  return fetch(`${BASE_URL}/get/eqpList`).then((response) =>
    response.json()
  );
}
...
export function fetchEventStreamList(eqpId: string) {
  return fetch(`${BASE_URL}/get/scenario/${eqpId}`).then((response) => response.json());
}
...
export function deleteEventLog(eqpId: string, logKey: string) {
  return fetch(`${BASE_URL}/manage/delete/eqpId/${eqpId}/logs/${logKey}`, { method: 'DELETE' }).then((response) =>
    response.json()
  );
}
